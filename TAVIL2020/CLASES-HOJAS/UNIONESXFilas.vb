...
Public Class UNIONESXFilas
    ...
    Public filas As List(Of UNIONESXFila)        ' Key = ITEM_NUMBER, Value = clsPTItem
    Public Sub New()
        'If cXML Is Nothing Then cXML = New ClosedXML2acad.ClosedXML2acad
        Campos = New List(Of String)
        filas = New List(Of UNIONESXFila)
        '
        For Each fila As ce.IXLRow In modClosedXMLTavil.Excel_LeeFilar(LAYOUTDB, nHoja, concabeceras:=True).AsParallel
            If fila.RowNumber = 1 Then
                ...
                Next
            Else
                filas.Add(New UNIONESXFila(fila))
            End If
    ...
    Public Function Fila_BuscaDame(inC As String, inInc As String, outC As String, outInc As String, angle As String) As UNIONESXFila
        ...
        For Each filtro As String In filtros
            Dim f1 As IEnumerable(Of UNIONESXFila) = From x In filas
    ...
    Public Function Fila_InclinationsInOut(inC As String, INFEED As Boolean) As String()
        ...
        Dim filtros() As String = {inC, inC.Substring(0, 6) & "XX"}
        For Each filtro As String In filtros
            Dim f1 As IEnumerable(Of UNIONESXFila) = Nothing
            If INFEED Then
